...
from smc.api.exceptions import UnsupportedEntryPoint, FetchElementFailed
...
class Search(object):
    ...
    def objects(self):
        """
        A collection resource for the element selected. Search parameter generates
        a dynamic class that is registered in the global registry when it is
        created. It will only be registered once for any given search filter. 
        ...
        if smc.base.model.lookup_class(self._resource) is smc.base.model.Element:
            ...
            # Class will be auto registered and then retrieved in the collection
            attrs={'typeof': self._resource}
            cls_name = '{0}Element'.format(self._resource.replace(',','_').title())
            element_cls = type(str(cls_name), (smc.base.model.Element,), attrs)
        else: 
            # Existing class of this type already exists, or it's a context filter
    ...
    def _validate(self, name):
        ...
        out elements not bound to the elements URI. Note: Search filters
        may be combined by using comma, such as Search('router,host'), so
        split out and check each one.
        """
        extracted_filters = name.split(',') #Multiple filters, format: 'router,host'
...
_context_filters = ('fw_clusters', 'engine_clusters', 'ips_clusters', 'layer2_clusters',
