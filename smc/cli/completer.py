...
class CommandCompleter(Completer):
    ...
    def get_completions(self, document, complete_event):
        ...
        if in_command:
            ...
            if (document.text and not command_name) and not word_before_cursor:
    ...
    def find_command_matches(command, word='', prev='', params=[]):
        ...
        if command:
            ...
            context_menu_items = all_sub_menus(top_cmd, sub_cmd) #menu's based on cmd and target context
            unused_menu_items = [(name, meta) for (name, meta) in context_menu_items \
