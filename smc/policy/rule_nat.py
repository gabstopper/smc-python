...
class NAT(object):
    ...
    def automatic_proxy(self):
        ...
        :param boolean value: enable/disable proxy arp
        :return: boolean
        """
        if self.has_nat:
    ...
    def original_value(self, value):
        ...
        if isinstance(value, Element):
            ...
            if value.startswith('http'): #href
                src = {'element': value}
        if src:
            self({'original_value': src})  
        #TODO: Raise here?
...
class IPv4NATRule(NATRule, SubElement):
    ...
    done either using the element href or element defined in 
    :py:class:`smc.elements.network` or :py:class:`smc.elements.service`.
