...
def create_host(name, ip, secondary_ip=[], comment=None):
    if smc.helpers.is_valid_ipv4(ip):
        ...
        logger.error("Failed: Invalid IPv4 address specified: %s, create host: %s failed" % (ip, name)) 
...
def create_group(group, members=[]):
    ...
    if members:
        for m in members: #add each member
            for member in smc.search.filter_by_element(m): #array of dict results
                if member['name'] == m:
                    print "Member found: %s, adding" % m
                    group_template['element'].append(member['href'])
    try:
...
def create_single_fw(name, mgmt_ip, mgmt_network, dns=None, fw_license=False):
    ...
    if fw_template:
        ...
        logger.error("Failed creating single_fw, template load failed")
...
if __name__ == '__main__':
    ...
    smc.create_group('group with no members')
    create_host('ami', '1.1.1.1')
    create_host('ami2', '2.2.2.2')
    create_group('anewgroup', ['ami','ami2'])
    smc.web_api.logout()
