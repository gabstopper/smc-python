...
def get_containers(all=True):  # @ReservedAssignment
    ...
    :param boolean all: show all containers; otherwise only
...
def get_container_stats(container_id, stream=False):
    ...
    :param boolean stream: stay connected to gather stats as stream (True)
...
def get_images(limit=False, filter=None):  # @ReservedAssignment
    ...
    :param boolean limit: True or False
...
def remove_container(container_id, volumes=True):
    ...
    :param boolean volumes: remove the volumes associated to the container
    """
